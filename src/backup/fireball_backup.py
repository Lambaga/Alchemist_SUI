...
class Fireball(pygame.sprite.Sprite):
    ...
    def load_animations(self, asset_path):
        """Load fireball animation frames using AssetManager"""
        ...
        try:
            ...
            if self.scale_factor != 1.0:
                # We'll scale based on original size
                target_size = None  # Let AssetManager handle individual scaling
            ...
            if os.path.exists(move_path):
                ...
                self.move_frames = self.asset_manager.load_spritesheet_frames(
                    move_path, num_frames=5, scale_to=target_size
                )
                
                # Apply scale factor if needed
                if self.scale_factor != 1.0:
                    scaled_frames = []
                    for frame in self.move_frames:
                        new_width = int(frame.get_width() * self.scale_factor)
                        new_height = int(frame.get_height() * self.scale_factor)
                        scaled_frame = pygame.transform.scale(frame, (new_width, new_height))
                        scaled_frames.append(scaled_frame)
                    self.move_frames = scaled_frames
