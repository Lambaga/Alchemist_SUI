...
from combat_system import DamageType
...
class Fireball(pygame.sprite.Sprite):
    ...
    def load_animations(self, asset_path):
        """Load fireball animation frames using AssetManager with configuration"""
        if not os.path.exists(asset_path):
            print("⚠️ Fireball asset path not found: {}".format(asset_path))
            return
            
        try:
            # Lade Animationen über die Konfiguration
            self.move_frames = self.asset_manager.load_entity_animation(
                'fireball', 'move', asset_path, self.scale_factor
            )
            
            # Explosion als einzelnes Bild laden
            explosion_path = os.path.join(asset_path, "Explosion.png")
            if os.path.exists(explosion_path):
                explosion_sheet = self.asset_manager.load_image(explosion_path)
                # Skalierung anwenden wenn nötig
            ...
            if not self.move_frames:
                self.move_frames = [self.create_placeholder()]
            if not self.explosion_frames:
                self.explosion_frames = [self.create_placeholder()]
        ...
        if self.state == "moving":
            ...
            if dt:
                ...
                if player and self.hitbox.colliderect(player.hitbox):
                    self.explode()
                    # Deal magical fire damage to player
                    damage_dealt = player.take_damage(self.damage, DamageType.FIRE)
                    if damage_dealt:
                        print("🔥 Fireball hit player for {} fire damage! HP: {}/{}".format(
                            self.damage, 
                            player.get_health(), 
                            player.get_max_health()
                        ))
                    else:
                        print("💀 Fireball killed the player!")
